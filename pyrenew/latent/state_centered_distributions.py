...
class StateAR1(Distribution):
    ...
    def sample(self, key: jax.Array, sample_shape: tuple[int, ...] = ()) -> ArrayLike:
        ...
        z0 = random.normal(keys[0], shape=per_step_shape)
        ...
        def step(
            prev: ArrayLike, key_t: jax.Array
        ) -> tuple[ArrayLike, ArrayLike]:  # numpydoc ignore=GL08
            z = random.normal(key_t, shape=per_step_shape)
            new = autoreg * prev + scale * z
            return new, new

        _, xs = lax.scan(step, x0, keys[1:])
...
class StateDifferencedAR1(Distribution):
    ...
    def sample(self, key: jax.Array, sample_shape: tuple[int, ...] = ()) -> ArrayLike:
        ...
        keys = random.split(key, self.num_steps)
        z1 = random.normal(keys[0], shape=per_step_shape)
        ...
        def step(
            carry: tuple[ArrayLike, ArrayLike], key_t: jax.Array
        ) -> tuple[tuple[ArrayLike, ArrayLike], ArrayLike]:  # numpydoc ignore=GL08
            prev_2, prev_1 = carry
            z = random.normal(key_t, shape=per_step_shape)
            new = prev_1 + autoreg * (prev_1 - prev_2) + scale * z
            return (prev_1, new), new

        _, xs = lax.scan(step, (initial_loc, x1), keys[1:])
