...
class PopulationInfections(BaseLatentInfectionProcess):
    ...
    ``rt_single`` is the raw population reproduction number generated by
    ``single_rt_process``. ``rt_single_effective`` is the reproduction number
    actually used by ``infection_process`` to generate infections. These are
    equal for plain renewal and can differ for infection processes with
    feedback or other adjustments.
    """
    ...
    def __init__(
        ...
        *,
        ...
        infection_process
            Infection process used to convert $\\mathcal{R}(t)$, initial
            infections, and generation interval into infections. Defaults to
            ordinary renewal via ``Infections``.
        ...
        self.infection_process = infection_process or Infections(name="infections")
    ...
    def sample(
        ...
        **kwargs: object,
        ...
        infections via exponential backprojection, and delegates the renewal
        equation to ``infection_process``. The temporal process may sample
        parameters at any supported cadence (for example daily or
        weekly/stepwise), but it must return a daily-length trajectory before
        the renewal equation is evaluated. The raw temporal-process Rt is
        recorded as ``rt_single``; the Rt used by ``infection_process`` is
        recorded as ``rt_single_effective``.
        ...
        **kwargs
        ...
        infection_sample = self.infection_process.sample(
        ...
        rt_single_effective = jnp.concatenate(
            [
                rt_single[: self.n_initialization_points, 0],
                infection_sample.rt,
            ]
        )[:, jnp.newaxis]
        ...
        with numpyro.handlers.scope(prefix=self.name, divider="::"):
            ...
            numpyro.deterministic("rt_single_effective", rt_single_effective)
