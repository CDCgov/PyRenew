...
class SubpopulationInfections(BaseLatentInfectionProcess):
    ...
    ``rt_baseline`` and ``rt_subpop`` are raw reproduction numbers generated by
    the temporal-process hierarchy. ``rt_subpop_effective`` is the reproduction
    number actually used by ``infection_process`` to generate infections. Raw
    and effective subpopulation Rt are equal for plain renewal and can differ
    for infection processes with feedback or other adjustments.
    """
    ...
    def __init__(
        ...
        *,
        ...
        name
            ...
            ``"{name}::rt_baseline"``, ``"{name}::rt_subpop"``, and
            ``"{name}::rt_subpop_effective"``).
        gen_int_rv
    ...
    def sample(
        ...
        **kwargs: object,
        ...
        deviations with a sum-to-zero constraint, initial infections, and
        delegates the renewal equations to ``infection_process``. The temporal
        ...
        trajectories before the renewal equations are evaluated. The raw
        subpopulation Rt is recorded as ``rt_subpop``; the Rt used by
        ``infection_process`` is recorded as ``rt_subpop_effective``.
        ...
        **kwargs
        ...
        rt_subpop_effective = jnp.vstack(
            [
                rt_subpop[: self.n_initialization_points, :],
                infection_sample.rt,
            ]
        )
        ...
        with numpyro.handlers.scope(prefix=self.name, divider="::"):
            ...
            numpyro.deterministic("rt_subpop_effective", rt_subpop_effective)
