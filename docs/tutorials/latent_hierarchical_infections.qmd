...
print(f"Number of subpopulations (K): {K}")
...
rt_samples = np.array(prior_samples["rt_baseline"])[:, n_init:, 0]
...
for name, temporal in temporal_configs.items():
    ...
    process_samples[name] = np.array(samples["rt_baseline"])[:, n_init:, 0]
```
...
for name, samples in tuning_samples.items():
    rt = np.array(samples["rt_baseline"])[:, n_init:, 0]
    for i in range(n_samples):
        for d in range(n_days):
            tuning_data.append(
                {
