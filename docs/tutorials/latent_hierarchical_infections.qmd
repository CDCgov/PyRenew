...
$$
I_k(t) = \mathcal{R}_k(t) \sum_{\tau=1}^{K_{\text{gen}}} I_k(t - \tau)\, w_\tau,
$$
...
Each subpopulation produces its own infection trajectory $I_k(t)$. These are combined into an aggregate infection process using population fractions $p_k$:

$$
I_{\text{aggregate}}(t) = \sum_{k=1}^{K} p_k \, I_k(t),
$$

where $p_k$ is the fraction of the total population in subpopulation $k$.
...
This ensures $\mathcal{R}_{\text{baseline}}(t)$ is the geometric mean of the subpopulation $\mathcal{R}_k(t)$ values, so the baseline represents the typical transmission level across subpopulations.
...
The deviations $\delta_k(t)$ are initialized at zero (all subpopulations start at the baseline) and evolve according to the chosen temporal process.
...
(
    ...
    + p9.labs(
        x="Days",
        y="Rt (baseline)",
        title="Baseline Rt (DifferencedAR1, autoreg=0.5, sd=0.01)",
    )
    + theme_tutorial
)
```
