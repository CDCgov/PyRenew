...
```{python}
# 5 observed subpopulations
obs_fractions = jnp.array([0.10, 0.14, 0.21, 0.22, 0.07])

# 1 unobserved subpopulation
unobs_fractions = jnp.array([0.26])

# Verify fractions sum to 1
total = float(jnp.sum(obs_fractions) + jnp.sum(unobs_fractions))
print(f"Total population fraction: {total:.2f}")
print(f"Observed coverage: {float(jnp.sum(obs_fractions)):.0%}")
print(f"Unobserved coverage: {float(jnp.sum(unobs_fractions)):.0%}")
```
...
print(f"Short GI mean: {float(np.sum(np.arange(len(gi_short)) * gi_short)):.1f} days")
print(f"Long GI mean: {float(np.sum(np.arange(len(gi_long)) * gi_long)):.1f} days")
```
...
baseline_temporal = DifferencedAR1(autoreg=0.9, innovation_sd=0.05)
subpop_temporal = AR1(autoreg=0.8, innovation_sd=0.05)
...
for init_rt_name, init_rt_val in [("Rt≈1.22 (growing)", 0.2), ("Rt≈0.82 (declining)", -0.2)]:
    model = HierarchicalInfections(
...
(
    p9.ggplot(gi_df, p9.aes(x="day", y="infections", color="config", linetype="linetype"))
    + p9.geom_line(size=1)
    + p9.scale_linetype_manual(
        values={"solid": "solid", "dashed": "dashed"},
        labels={"solid": "initial Rt = 1", "dashed": "initial Rt ≠ 1"},
    )
    + p9.labs(
        x="Days",
...
(
    p9.ggplot(gi_df_zoom, p9.aes(x="day", y="infections", color="config", linetype="linetype"))
    + p9.geom_line(size=1)
    + p9.scale_linetype_manual(
        values={"solid": "solid", "dashed": "dashed"},
        labels={"solid": "initial Rt = 1", "dashed": "initial Rt ≠ 1"},
...
initial_log_rt_rv = DeterministicVariable("initial_log_rt", 0.0)  # Rt starts at 1.0
...
In the previous plot, `I0` was specified as:  `I0_rv=DeterministicVariable("I0", 0.001)`, i.e., $1%$ of the population.  We overplot trajectories for $10%$ and $0.1%$ as well.
...
print(f"Generation interval length: {latent_infections.get_required_lookback()} days")
print(f"Initialization points: {latent_infections.n_initialization_points} days")
```
...
infections_agg, infections_all, infections_obs, infections_unobs = result
...
print(f"  Observed subpopulations: {infections_obs.shape}")
print(f"  Unobserved subpopulations: {infections_unobs.shape}")
...
print(f"Rt baseline samples: {prior_samples['rt_baseline'].shape}")
print(f"Infection samples: {prior_samples['infections_aggregate'].shape}")
```
...
rt_samples = np.array(prior_samples["rt_baseline"])[:, n_init:]
...
temporal_configs = {
    ...
    "DifferencedAR1": DifferencedAR1(autoreg=0.9, innovation_sd=0.05),
...
for name, temporal in temporal_configs.items():
    ...
    process_samples[name] = np.array(samples["rt_baseline"])[:, n_init:]
```
...
(
    ...
    + p9.labs(
        ...
        subtitle="Same innovation_sd=0.05; AR processes have autoreg=0.9",
    )
    + p9.theme_minimal()
...
for name in ["AR(1)", "DifferencedAR1", "RandomWalk"]:
    ...
    print(f"{name:<18} {np.median(rt):>8.2f} {np.mean(rt):>8.2f} "
...
deviation_samples = {}
for name, subpop_temporal in subpop_configs.items():
    model = HierarchicalInfections(
        gen_int_rv=gen_int_rv,
        I0_rv=I0_rv,
        initial_log_rt_rv=initial_log_rt_rv,
    ...
    deviation_samples[name] = Predictive(make_sampler(model), num_samples=n_samples)(
        random.PRNGKey(42)
    )
```
...
for name in subpop_configs.keys():
    devs = np.array(deviation_samples[name]["subpop_deviations"])[:, n_init:, 0]
    for i in range(n_samples):
        for d in range(n_days):
            dev_data.append({
                "day": d,
...
for name, cfg in configs.items():
    model = HierarchicalInfections(
        ...
        I0_rv=I0_rv,
        initial_log_rt_rv=initial_log_rt_rv,
        baseline_temporal=cfg["baseline"],
        subpop_temporal=cfg["subpop"],
    )
    ...
    tuning_samples[name] = Predictive(make_sampler(model), num_samples=n_samples)(
        random.PRNGKey(42)
    )
```
...
for name, samples in tuning_samples.items():
    rt = np.array(samples["rt_baseline"])[:, n_init:]
    for i in range(n_samples):
        for d in range(n_days):
            tuning_data.append({
                "day": d,
...
print(f"Max |sum of deviations|: {float(jnp.max(jnp.abs(deviation_sums))):.2e}")
...
with numpyro.handlers.seed(rng_seed=123):
    inf_agg, inf_all, _, _ = latent_infections.sample(
        n_days_post_init=n_days,
        obs_fractions=obs_fractions,
        unobs_fractions=unobs_fractions,
    )

all_fractions = jnp.concatenate([obs_fractions, unobs_fractions])
computed_agg = jnp.sum(inf_all * all_fractions[None, :], axis=1)
max_diff = float(jnp.max(jnp.abs(computed_agg - inf_agg)))

print(f"Max difference (aggregate vs weighted sum): {max_diff:.2e}")
print("Hierarchical structure verified!" if max_diff < 1e-5 else "ERROR: Mismatch detected")
```
...
- **`model_architecture.qmd`**: Learn how to combine `HierarchicalInfections` with observation processes using `ModelBuilder`
- **Observation process tutorials**: `observation_processes_counts.qmd` and `observation_processes_measurements.qmd`
...
infections_agg, infections_all, infections_obs, infections_unobs = latent.sample(
    n_days_post_init=28,
    obs_fractions=obs_fractions,
    unobs_fractions=unobs_fractions,
)
...
In a full model, you would pass `HierarchicalInfections` to `ModelBuilder` along with observation processes—the builder handles initialization, routing, and assembly automatically.
