...
AR(1) deviations have **bounded variance**. Because every subpopulation starts at the baseline ($\delta_k(0) = 0$), there is no initial displacement to "revert from" — the visible behavior in a prior predictive plot is the spread of $\delta_k(t)$ growing from $0$ at initialization toward a stationary value $\sigma^2 / (1 - \phi^2)$ and then staying there. With `autoreg = 0.8` and `innovation_sd = 0.05`, that stationary standard deviation is approximately $0.083$ on the log scale, reached after roughly 10 days.
...
# | fig-cap: AR(1) deviation trajectories (50 samples, all 6 subpopulations). The envelope grows from zero at initialization to its stationary width within roughly 10 days, then stays bounded. Compare the y-axis scale to fig-rw-deviations below.
(
    p9.ggplot()
    + p9.geom_line(
        ar1_dev_df,
