...
The generation interval is fixed by pathogen biology and is not estimated from data. For COVID-19, most transmission occurs within the first few days. We define a 7-day PMF based on estimates from the literature:
...
# | fig-cap: COVID-like generation interval distribution. Most secondary infections occur within 1-3 days of the index infection.
...
The generation interval length determines the minimum initialization period: the renewal equation at day 0 needs infections from the preceding $S - 1$ days, where $S$ is the length of the generation interval PMF.
...
# | fig-cap: 'Side-by-side comparison of all three temporal processes with hyperparameters chosen to produce comparable trajectory envelopes. The qualitative differences are in trajectory shape: AR(1) reverts to Rt = 1, DifferencedAR1 sustains trends, Random Walk drifts freely.'
(
