...
```{python}
# | label: fig-ar1
# | fig-cap: Prior predictive Rt trajectories under AR(1) (autoreg = 0.9, innovation_sd = 0.05). The envelope stabilizes rather than growing over time. The median trajectory (solid line) drifts from the starting Rt of 1.65 back toward 1, illustrating mean reversion of the level.
rt_spaghetti_plot(
...
# | tbl-cap: Rt at day 28 by temporal process.
summary_rows = []
...
# | tbl-cap: Rt at day 28 by DifferencedAR1 configuration.
hp_summary_rows = []
for name in hyperparam_configs:
