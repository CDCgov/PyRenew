...
flowchart LR
    ...
    subgraph observations["Observations module"]
        obs["infection_obs_process_rv (PoissonObservation)"]
    end

    subgraph latent["Latent module"]
        inf["latent_infections_rv (Infections)"]
        i0["I0_rv (DistributionalVariable)"]
    end

    subgraph process["Process module"]
        rt["Rt_process_rv (Custom class built using RandomWalk)"]
    end

    subgraph deterministic["Deterministic module"]
        detpmf["gen_int_rv (DeterministicPMF)"]
    end

    subgraph model[Model module]
        model1["model1 (RtInfectionsRenewalModel)"]
    end
