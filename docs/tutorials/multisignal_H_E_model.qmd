...
This tutorial builds a small hospital + emergency department model using PyRenew's high-level components and `PyrenewBuilder` class to specify a `MultiSignalModel` object,
then fits it to a synthetic dataset.

We then compare this PyRenew model with the production model used to produce weekly short-term forecasts of Covid, flu, and RSV for the [CDC ensemble forecast hubs](https://
www.cdc.gov/cfa-modeling-and-forecasting/covid19-data-vis). The production model is coded directly in NumPyro and uses a more specialized H+E ascertainment structure, so the
comparison below is a structural comparison rather than a direct benchmark.
...
- **Time-varying ED ascertainment with a hospital-to-ED link**: the production H+E model uses a time-varying ED ascertainment rate and defines scalar hospital ascertainment
  as a multiplier of the first ED ascertainment value. In this tutorial model, `JointAscertainment` instead samples two scalar rates from one joint prior.
...
We use a common generic baseline rate and a broad logit-scale covariance.
The prior correlation of $0.5$ says that the two rates may differ,
but prior draws that are high for hospital ascertainment also tend to be high for ED ascertainment.

```python
ascertainment_center = 0.004
ascertainment_sd = 1.0
...
The pyrenew-H-E model learns the structure of the synthetic data rather than recovering every data-generating parameter. Its posterior $\mathcal{R}(t)$ trajectory follows the
true falling-rising-falling pattern, and the fitted hospital and ED signals match the observations well. However, with a diffuse ascertainment prior, the absolute IHR and
IEDR can shift away from their data-generating values because H+E observations weakly identify the absolute infection scale.
...
The latent infection interval should be interpreted as a scale diagnostic, not just a coverage check: with diffuse ascertainment, uncertainty in the absolute infection-to-observation rates propagates directly into uncertainty in latent infections.
...
The PyRenew model and prod-H-E use different ascertainment structures, so this table is a compact way to compare their implied absolute scales rather than a direct test of which implementation is correct.
...
Both fits are useful diagnostics for scale and structure, not a contest between implementations.
...
#| fig-cap: |-
#|   Posterior densities for $I_0$, IHR, and IEDR under pyrenew-H-E and
...
If the PyRenew fit lies closer to the synthetic truth for some quantities, that should be interpreted in light of the data-generating process.
The synthetic data were generated with scalar hospital and ED ascertainment rates, which is closer to the `JointAscertainment` model used here than to the production H+E model.
The production model allows ED ascertainment to vary over time and defines hospital ascertainment through a multiplier on the first ED ascertainment value.
Those features are useful for production forecasting, but they are additional structure relative to this simplified synthetic generator.
