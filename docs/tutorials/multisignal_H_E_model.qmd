...
We then compare this PyRenew model with the production model used to produce weekly short-term forecasts of Covid, flu, and RSV for the [CDC ensemble forecast hubs](https:// www.cdc.gov/cfa-modeling-and-forecasting/covid19-data-vis).
The production model is coded directly in NumPyro and uses a more specialized H+E ascertainment structure, so the comparison below is a structural comparison rather than a direct benchmark.
...
- **Time-varying ED ascertainment with a hospital-to-ED link**: the production H+E model uses a time-varying ED ascertainment rate and defines scalar hospital ascertainment as a multiplier of the first ED ascertainment value.
