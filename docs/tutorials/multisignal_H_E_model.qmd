...
For ED visits, the observation model can include a day-of-week effect.
...
The custom H+E model estimates a seven-element day-of-week multiplier using a symmetric Dirichlet prior with concentration parameter $5$ on each day, then rescales the draw by $7$ so the multipliers have mean $1$, encoding a mild prior belief that day-of-week effects are modest deviations from a flat week.

In this tutorial, we fix the ED day-of-week multipliers to the synthetic data-generating values.
This keeps the PyRenew example focused on the mixed daily/weekly H+E structure and avoids adding another weakly identified observation-process component.
...
A closer structural match to custom-H+E would estimate these multipliers with a scaled Dirichlet prior:
...
The ED visit observation process is defined as follows:
...
The `ascertainment.for_signal("ed_visits")` connects the ED observation process to the ED component of the joint ascertainment model.
...
pyrenew_ratio_summary = az.summary(
    az.from_dict(
        {"posterior": {"IHR_over_IEDR": pyrenew_ratio.values}},
        sample_dims=["chain", "draw"],
    )
)
pyrenew_ratio_summary
```

The table below compares posterior medians to the data-generating values.
The final column is a scale comparison: values near $1$ indicate close recovery of that scalar quantity.

```{python}
#| label: posterior-truth-comparison

def posterior_median(draws):
    return float(draws.median(dim=["chain", "draw"]))
...
posterior_comparison = pd.DataFrame(
    [
        {
            "quantity": "I0",
            "data-generating value": true_i0,
            "posterior median": posterior_median(idata_trimmed.posterior["I0"]),
        },
        {
            "quantity": "IHR",
            "data-generating value": true_ihr,
        ...
        {
            "quantity": "IEDR",
            "data-generating value": true_iedr,
            "posterior median": posterior_median(
                idata_trimmed.posterior["he_ascertainment_ed_visits"]
            ),
        },
        {
            "quantity": "IHR / IEDR",
            "data-generating value": true_ihr / true_iedr,
            "posterior median": posterior_median(pyrenew_ratio),
        },
    ]
)

posterior_comparison["posterior median / truth"] = (
    posterior_comparison["posterior median"]
    / posterior_comparison["data-generating value"]
)

posterior_comparison.round(4).style.hide(axis="index")
```

The pyrenew-H-E model learns the structure of the synthetic data rather than recovering every data-generating parameter.
With a diffuse ascertainment prior, the absolute IHR and IEDR can shift away from their data-generating values because H+E observations weakly identify the absolute infection scale.
...
pyrenew_rt_summary.index = [f"Rt day {day}" for day in rt_days]
pyrenew_rt_summary
```
...
```{python}
#| label: fig-all-model-rt
#| fig-cap: Posterior Rt under the two PyRenew parameterizations and custom-H-E. The dashed black line is the synthetic data-generating Rt trajectory.
...
```{python}
#| label: latent-infection-interval-width
...
We compare the relative width of the posterior interval across all three models.

```{python}
#| label: fig-latent-infection-interval-width
#| fig-cap: Relative posterior interval width for latent infections over the synthetic H+E fit.
...
sampler_diagnostics.style.hide(axis="index")
```

```{python}
#| label: fig-linked-ratio-prior-posterior
...
latent_width_summary.style.hide(axis="index")
```

```{python}
#| label: fig-linked-latent-infections
#| fig-cap: Posterior latent infections under the two PyRenew parameterizations and custom-H-E. The dashed black line is the synthetic data-generating latent infection trajectory, not an observed signal used for fitting.
...
```{python}
#| label: fig-linked-latent-infection-uncertainty
#| fig-cap: Posterior uncertainty intervals over infections under the two PyRenew parameterizations and custom-H-E.
...
inference_comparison.style.hide(axis="index")
```
...
```{python}
#| label: fig-identifiability-scale-densities
#| fig-cap: Posterior densities for $I_0$, IHR, and IEDR under all three models. Red dashed lines mark data-generating values.
...
```{python}
#| label: fig-identifiability-ihr-iedr
#| fig-cap: Joint posterior of hospital and ED ascertainment rates under both PyRenew parameterizations and custom-H-E.
...
```{python}
#| label: fig-identifiability-ratio
#| fig-cap: Posterior distribution of the IHR / IEDR ratio under all three models.
