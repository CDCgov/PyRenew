...
The custom H+E model estimates a seven-element day-of-week multiplier using a symmetric Dirichlet prior with concentration parameter $5$ on each day, then rescales the draw by $7$ so the multipliers have mean $1$,
encoding a mild prior belief that day-of-week effects are modest deviations from a flat week.
...
The ED visit observaton process is defined as follows:
...
With a diffuse ascertainment prior, the absolute IHR and IEDR can shift away from their data-generating values
because H+E observations weakly identify the absolute infection scale.
...
```
*Figure: Posterior Rt under the two PyRenew parameterizations and custom-H-E.
...
```
*Figure: Relative posterior interval width for latent infections over the synthetic H+E fit.*
...
Instead of sampling hospital and ED ascertainment as two rates from one joint prior,
it samples ED ascertainment as a base rate and samples the multiplier `IHR / IEDR` directly.
...
```
*Figure: Prior and posterior for the linked PyRenew IHR / IEDR multiplier.*
...
```
*Figure: Posterior latent infections under the two PyRenew parameterizations and custom-H-E. The dashed black line is the synthetic data-generating latent infection trajectory, not an observed signal used for fitting.*
...
```
*Figure: Posterior uncertainty intervals over infections under the two PyRenew parameterizations and custom-H-E.*
...
```
*Figure: Posterior densities for $I_0$, IHR, and IEDR under all three models.
...
```
*Figure: Joint posterior of hospital and ED ascertainment rates under both
...
```
*Figure: Posterior distribution of the IHR / IEDR ratio under all three models.*
