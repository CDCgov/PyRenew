...
max_days = max(len(hosp_delay_pmf), len(ed_delay_pmf)) - 1
x_limits = (0, max_days)

plot_hosp_delay = plot_delay_distribution(
    hosp_delay_pmf,
    "Infection-to-Hospitalization Delay Distribution",
) + p9.coord_cartesian(xlim=x_limits)
plot_ed_delay = plot_delay_distribution(
    ed_delay_pmf,
    "Infection-to-ED Visit Delay Distribution",
) + p9.coord_cartesian(xlim=x_limits)

display(plot_hosp_delay)
display(plot_ed_delay)
```
