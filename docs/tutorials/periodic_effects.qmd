...
The `RtPeriodicDiff` and `RtWeeklyDiff` classes use `PeriodicBroadcaster` to repeat each vector element to represent Rt values held constant within a period. Let's show its usage with a basic renewal model. We will use the same model structure shown in the `getting_started` tutorial:
