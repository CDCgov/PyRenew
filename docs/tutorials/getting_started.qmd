...
At this stage, the `Model` metaclass consists of two model classes: (1) `RtInfectionsRenewalModel`, which is basic renewal model consisting of infections and reproduction numbers. (2) `HospitalAdmissionsModel`, which includes basic renewal model and hospital admissions. In the subsequent sections, we provide examples of fitting each of these models.
