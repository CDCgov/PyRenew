...
In a Bayesian model, all quantities — data, parameters, hyperparameters, derived computations —
are random variables living in a single joint probability model (Gelman et al., BDA3 §1.3).
...
so you can swap a fixed quantity for an estimated one — or vice versa —
without changing any surrounding model code.
...
you can swap them freely. For example, the `PopulationCounts` observation process
...
We now use the `AscertainmentWithDayOfWeek` instance defined above
to generate day-of-week multipliers within a realistic observation
pipeline, showing how a custom `RandomVariable` integrates into a
...
The day-of-week effect is a *reporting* artifact, so it is
applied **after** the delay convolution.
...
We convolve infections with the delay PMF and apply a baseline
ascertainment rate to get the expected admissions before noise:
...
With effects, we first call our custom `ascertainment_rv` to get a
day-of-week multiplier, apply it to the expected admissions
(post-convolution, at the reporting stage), then add noise.
...
The right panel shows the additional weekly oscillation introduced by
day-of-week reporting effects — the sawtooth pattern of weekend dips
and weekday peaks that is characteristic of real hospital admissions data.
