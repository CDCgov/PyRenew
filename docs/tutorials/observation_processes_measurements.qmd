...
where $f(\cdot)$ is a transformation (often the identity or log function), and the noise model adds stochastic variation around this transformed expectation.
...
The expected concentration is determined by:
...
The expected concentration at time $t$ is given by

$$
\mu(t) = \frac{G}{V} \sum_{\delta=0}^{D} I(t-\delta)\, \pi_\delta,
$$

where:

- $I(t-\delta)$ is the number of infections on day $t-\delta$
- $\pi_\delta$ is the shedding kinetics PMF (fraction of total shedding occurring $\delta$ days after infection)
...
- A **deterministic layer**, where the convolution defines the expected concentration $\mu(t)$ from latent infections
