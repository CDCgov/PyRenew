...
For wastewater, `subpop_indices` links each observation to the appropriate subpopulation, while `sensor_indices` selects the sensor-specific noise parameters.
...
The returned dictionary is structured to match the keyword arguments of `model.run()`: `obs_start_date` at the top level, plus one sub-dictionary per observation process.
...
At call time, this dictionary is unpacked with `**`, forwarding each entry as a keyword argument to `model.run()`.
...
In the fits below, we use 4 chains and turn off the progress bar:
...
We now prepare the observation dictionaries described above and unpack them into `model.run()`.
The first fit uses 90 days of data.
...
The 90-day and 180-day fits also show why the end of the observation window matters.
