...
::: {.callout-warning}
## Nest observation-specific arguments under the signal name

Arguments such as `subpop_indices` must be placed inside the dictionary for the corresponding registered observation process.
Passing them at the top level does not associate them with that signal and raises a validation error before MCMC begins.

```python
# Correct: observation-specific metadata is nested under the signal name.
model.run(
    ...,
    regional_counts={
        "obs": counts,
...
# Incorrect: this top-level argument is not associated with regional_counts.
model.run(
    ...,
    regional_counts={"obs": counts},
    subpop_indices=subpop_indices,
)
```
:::
