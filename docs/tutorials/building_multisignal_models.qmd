...
flowchart TB
    ...
    subgraph Infections["Infection Trajectories"]
        ...
        S["Per-subpopulation infections<br/>(all K subpopulations)"]
    end
...
| `"subpop"` | Infection matrix for all K subpopulations, shape `(T, K)` | Wastewater, site-specific surveillance |
...
**Understanding `subpop_indices`**: The latent process generates infections for all K subpopulations as a matrix of shape `(T, K)`. Each observation selects which column (subpopulation) it came from using `subpop_indices`. This is how observation processes "know" which subpopulations they observe—the user specifies this mapping at sample/fit time.
...
-   `subpop_indices` links each observation to the appropriate infection column (0-indexed into the K subpopulations)
...
Population structure is specified via a single array of fractions for all K subpopulations:

```python
model.fit(
    subpop_fractions=jnp.array([0.10, 0.14, 0.21, 0.22, 0.07, 0.26]),  # 6 subpopulations
    ...
)
```
...
model.fit(
    ...
    # Population structure (all K subpopulations)
    ...
    wastewater={
        ...
        "subpop_indices": ww_subpop_indices,  # selects which of K subpops each obs came from
...
First we declare the population structure. We have 6 subpopulations, where 5 have wastewater monitoring and 1 does not. The subpopulations with wastewater monitoring need not be contiguous indices—they could be any subset of {0, 1, ..., K-1}.
