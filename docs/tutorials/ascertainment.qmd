...
$$
\eta_j(t) = \operatorname{logit}(\alpha_{j,0}) + z_j(t),
\qquad
\alpha_j(t) = \operatorname{logit}^{-1}(\eta_j(t)).
$$

Here $\alpha_{j,0}$ is the natural-scale baseline ascertainment rate from `baseline_rates`, and $z_j(t)$ is a temporal deviation on the logit scale.
