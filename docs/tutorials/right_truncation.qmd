...
where $F$ is the CDF of the reporting delay and $k_t = (T - t) + \text{offset}$ is the number of days between timepoint $t$ and the data pull date.
Here $T$ is the last observation day and $\text{offset}$ is the number of additional days between day $T$ and the data pull (the `right_truncation_offset` parameter).

Timepoints far in the past have large $k_t$, so $F(k_t) = 1$ (fully reported).
Recent timepoints have small $k_t$ and $F(k_t) < 1$, reducing the predicted counts to reflect incomplete reporting.
...
The two panels below show predicted admissions with and without right-truncation, stacked so the time axes align.
...
(
    p9.ggplot(pred_df, p9.aes(x="day", y="admissions"))
    + p9.geom_line(size=1, color="steelblue")
...
The top panel shows admissions under complete reporting — they track the epidemic's growth.
The bottom panel shows what the data system actually delivers: recent counts are depressed because reports have not yet arrived.
Without the right-truncation adjustment, a model fit to the bottom curve would infer that the epidemic is slowing down — a dangerous misinterpretation during an active outbreak.
...
(
    ...
    + p9.labs(
        ...
        title="Sampled Observations: With vs. Without Right-Truncation",
    )
    + p9.theme_grey()
    + p9.theme(plot_title=p9.element_text(size=14, weight="bold"))
)
