---
title: 'Model Building: Composing Latent and Observation Processes'
format:
  gfm:
    fig-width: 16
    fig-height: 10
...
The `pyrenew` package provides a modular architecture for building multi-signal renewal models by composing:

-   **Latent infection process**: Generates unobserved infection trajectories via the renewal equation
-   **Observation processes**: Transform latent infections into observable signals (hospital admissions, wastewater, etc.)

This notebook demonstrates building a renewal model using the following components:
...
-   `Wastewater` — viral concentrations (site-level)

The `ModelBuilder` class composes these components and handles initialization requirements automatically.

## Observation Processes for Hospital Admissions and Wastewater Concentrations

Observation processes transform latent infections into observable signals and define the likelihood. Each observation process:

-   Declares what **infection resolution** it needs (`"aggregate"` or `"site"`)
-   Applies signal-specific transformations (ascertainment, delay convolution, shedding kinetics)
-   Defines the noise model for the likelihood
...
The name passed to add_observation() becomes the keyword used to pass that observation's data to model.fit() (e.g., hospital={...}, wastewater={...}).

```{python}
builder.add_observation(hosp_obs, "hospital")
builder.add_observation(ww_obs, "wastewater")
model = builder.build()
...
### Data Routing - Mapping Signals to Model Levels

This is a hierarchical model that estimates parameters at two levels:

- **Jurisdiction level**: a baseline $R(t)$ trajectory and aggregated infections across the entire jurisdiction
- **Subpopulation level**: deviations from the baseline $R(t)$ for each subpopulation, producing separate infection trajectories

Different signals observe different levels of this hierarchy. Each observation process declares an **infection resolution** that determines what infection data it receives:
...
| `"site"` | Infection matrix with one column per observed subpopulation | Wastewater, site-specific surveillance |

The model routes latent infections to observation processes based on each process's declared resolution:
...
Each observation process is registered with a name via `add_observation()`. That name becomes the keyword argument for passing data to `model.fit()`:

```python
builder.add_observation(hosp_obs, "hospital")    # → hospital={...}
builder.add_observation(ww_obs, "wastewater")    # → wastewater={...}
```
...
*Note*: this example uses dense data, i.e., there is an obervation for every day in the timeline. Were the data sparse, it would be necessary to also provide an array `times` which supplies the time index for each observation.
...
print(f"\nPosterior samples shape: {lat_inf_90.shape}")
...
Finally, we visualize the posterior latent infections over the 90 day interval.

```{python}
# Visualize posterior latent infections (90 days)
posterior_df_90 = pd.DataFrame(
    {
...
obs_plot_df_90 = pd.DataFrame(
    {
        "day": np.arange(n_days_90),
        "admissions": np.array(hosp_admits[:n_days_90]),
    }
)
...
(
    ...
    + p9.labs(
        x="Day",
        y="Count",
        title="Posterior Latent Infections (90 days, multi-signal fit)",
        subtitle="Blue: posterior infections (90% CI) | Orange: observed hospital admissions",
    )
    + p9.theme_grey()
)
```
...
print(f"\nPosterior samples shape: {lat_inf_180.shape}")
...
```{python}
# Visualize posterior latent infections (180 days)
posterior_df_180 = pd.DataFrame(
    {
...
obs_plot_df_180 = pd.DataFrame(
    {
        "day": np.arange(n_days_180),
        "admissions": np.array(hosp_admits[:n_days_180]),
    }
)
...
(
    ...
    + p9.labs(
        x="Day",
        y="Count",
        title="Posterior Latent Infections (180 days, multi-signal fit)",
        subtitle="Blue: posterior infections (90% CI) | Orange: observed hospital admissions",
    )
    + p9.theme_grey()
)
...
We can compare how the posterior estimates differ between fitting to 90 days vs 180 days of data. The key insight is how the credible intervals change in the overlapping period (first 90 days) when we have more data informing the model.
...
This notebook demonstrated composing a multi-signal renewal model using `ModelBuilder`:
