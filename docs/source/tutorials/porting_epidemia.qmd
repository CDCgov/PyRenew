...
\lettrine[lines=5, depth=0]{O}{ne} aim of the Multisignal Signal Renewal (hereafter MSR[^msr_v_pyrenew]) project is to make [statistical inference](https://en.wikipedia.org/wiki/Statistical_inference) and [simulation](https://en.wikipedia.org/wiki/Simulation) within the context of infectious disease modeling more flexible, relative to existing frameworks. Nonetheless, MSR users might want to re-implement existing, external (as in not pre-packaged) infectious disease models. Such capabilities are possible with MSR.
...
In this tutorial, the author demonstrates how to implement an influenza forecasting model (hereafter `cfaepim`[^cfaepim]) that was created using the [Epidemia](https://github.com/CDCgov/multisignal-epi-inference) framework. The author later compares the two models (`cfaepim` and the re-implementation in MSR) using daily resolution [NHSN](https://healthdata.gov/Hospital/COVID-19-Reported-Patient-Impact-and-Hospital-Capa/g62h-syeh)[^flusight] influenza hospitalization data. From this comparison, several enhancements to the MSR code base are identified and later made to reconcile performance differences.
...
[^cfaepim]: The `cfaepim` model is contained within a private [CDC Enterprise](https://github.com/cdcent) repository. However, the author of this tutorial has received permission from [the author](https://github.com/dylanhmorris) of the repository to reproduce the parts of the model code.

__Overview__: The author first outlines some _know-requisites_[^itali], i.e. activities and terms that should be completed and understood, respectively, prior to the completion of this tutorial. Next, the author showcases an example `cfaepim` forecast, along with some historical influenza hospitalization data. After this, the author briefly describes Epidemia and the class of problems it is aimed at handling. Then, one at a time, the author mathematically details components of `cfaepim`, providing motivating examples in Python and translations into MSR terms. Finally, the author compares the influenza hospitalization forecast output produced by the `cfaepim` instantiation in MSR with the output of the `cfaepim` for the same target weeks.

[^itali]: Block quotes or lengthy italicized sections in this tutorial are very unlikely to be the author's own writing; instead, if no attribution is provided, it is safe to assume the content originated from `cfaepim` code or from descriptions that can be found on Epidemia's website.
...
```
git+https://github.com/CDCgov/
multisignal-epi-inference@main#subdirectory=model
```
The author believes it is reasonable for the reader to proceed through _at least_ one of the following tutorials before continuing. Find below some MSR tutorials, ranging from low to high, starting from the first entry, of background knowledge required:
...
The `cfaepim` repository was employed during the 2023-24 respiratory season to forecast weekly influenza hospitalizations in US territories. For certain weeks during the respiratory season, some territories (e.g., Kansas) experienced major reporting delays and errors. `cfaepim` was built to handle reporting anomalies—the user is able to select start and cutoff intervals, in addition to exclusion intervals, for particular territories and epiweeks.

As an example, take the `2024-03-30` target reporting date, where data from the following epiweeks were excluded from fitting for Arizona, due reporting inadequacies:
...
Below find some `cfaepim` forecasts:
...
Further, the following pages on Epidemia might aid the reader in their understanding:
...
_The following description is a slightly reworded version of some of Epidemia's introduction page, geared towards influenza hospitalization forecasting. This problem statement attempts to further provide the reader with an intuition for how Epidemia is used in infectious disease modelling._

Suppose, over some interval of time, we received reported daily influenza hospitalization counts for a US territory; these influenza hospitalizations may be represented as $$Y = (Y_1, \dotsc, Y_n)$$

where $n$ denotes the $n$-th time step. Further, $Y_t$, i.e. the influenza hospitalization count for day $t$ can be...
...
The aforementioned Epidemia schematics page is worth looking at; see the Appendix.

# Epidemia For Influenza And MSR Equivalent

The `cfaepim` model consists of pre- and post-processing components that interface with a model instantiated via Epidemia. This Epidemia model is separated into three sections: (1) a transmission component, (2) an observation component, and (3) a function that integrates the other Epidemia-related components, which includes the infections component.

<!-- These three sections can be found across the code in `model.R`, which is detailed holistically in the Appendix. -->

## Parameters & Priors

The variables and their values below are referenced in subsequent sections. Skimming these values now might prove useful. The author of this tutorial cannot currently justify the values of these variables, but is in the process of obtaining justifications. The `cfaepim` code in this section was originally contained in a `toml` configuration file; parameter values are accessed via `params$<config_param>` in upcoming code blocks.
...
With a more thorough understanding of the observation process, let us turn our attention to the infection process.

NOTE: The infections component code is contained within the `build_state_light_model()` function in `model.R`. The code for this entire function appears in the next section.
...
```yaml
title: "Implementing Existing Epidemiology Models Using MSR"
subtitle: "A Tutorial Example Porting Epidemia"
author: "UPX3"
date: "2024 June 09"
geometry:
- lmargin=1.5in
...
- \usepackage{lettrine}
- \newfontface\customfont[Path=./source/fonts/]{GoudyInitialen.ttf}
- \renewcommand{\LettrineFontHook}{\customfont}
- \usepackage{sectsty}
...
## Full Mathematical Outline

Using information for AL:
...
& \beta_{1,1}, \dotsc, \beta_6 \sim \mathcal{N}(0, 0.25) \\
...
$$
\begin{aligned}
Y &= (Y_1, \dotsc, Y_n), \quad Y_t \in \mathbb{N}, \; t \in \{1, 2, \dots, n\} \\
Y_t &\sim \mathcal{D}(y_t, \phi), \quad t > 0 \\
\mathcal{D} &= \text{NegBin} \\
\phi &\sim \mathcal{N}(10, 5) \\
y_t &= \alpha_t \sum_{s<t} i_s \pi_{t-s}, \quad t > 0 \\
\alpha_t &= g_1^{-1}(\eta_{1,t}), \quad \alpha_t > 0 \\
g_1^{-1}(x) &= \frac{1}{1 + e^{-x}}, \quad g_1^{-1}: \mathbb{R} \to (0, 1) \\
\eta_{1,t} &= \beta_{0,1} + \sum_{i=1}^9 \beta_{i,1} x_i, \quad t > 0 \\
\beta_{0,1} &\sim \mathcal{N}(-6.5, 0.5) \\
\beta_{1,1}, \dotsc, \beta_{6,1} &\sim \mathcal{N}(0, 0.25) \\
\beta_{7,1} &\sim \mathcal{N}(-0.2, 0.2) \\
\beta_{8,1} &\sim \mathcal{N}(0, 0.2) \\
\beta_{9,1} &\sim \mathcal{N}(-50, 0.01) \\
\pi &= \begin{bmatrix} 0.05 & 0.1 & 0.175 & 0.225 & 0.175 & 0.125 & 0.075 & 0.05 & 0.025 \end{bmatrix}, \quad \pi_k \geq 0, \; \sum_{k=1}^9 \pi_k = 1 \\
i_t' &= \mathscr{R}_t \sum_{s < t} i_s g_{t-s}, \quad i_t \in \mathbb{R}_{\geq 0}, \; t > 0 \\
i_{v:0}, \mathscr{R}, \phi, \alpha &\sim p(\cdot), \quad v \leq 0 \\
\mathscr{R}_t &= g_2^{-1}(\eta_{2,t}), \quad \mathscr{R}_t \geq 0 \\
g_2^{-1}(x) &= \frac{K}{1 + e^{-x}}, \quad g_2^{-1}: \mathbb{R} \to (0, K) \\
\eta_{2,t} &= \beta_{0,2} + W_t, \quad t > 0 \\
\beta_{0,2} &\sim \mathcal{N}(-0.4054651, 0.3) \\
W_0 &= 0 \\
W_t &= W_{t-1} + \gamma_t, \quad t > 0 \\
\gamma_t &\sim \mathcal{N}(0, \sigma) \\
\sigma &\sim \mathcal{N}^{+}(0, 0.25) \\
\mathscr{R} &= (\mathscr{R}_1, \dotsc, \mathscr{R}_n), \quad \mathscr{R}_t \in \mathbb{R}_{\geq 0}, \; t > 0 \\
\alpha &= (\alpha_1, \dotsc, \alpha_n), \quad \alpha_t > 0, \; t > 0 \\
p(i_{v:0}, \mathscr{R}, \phi, \alpha ∣ Y) &\propto p(i_{v:0}) p(\mathscr{R}) p(\phi) p(\alpha) \prod_t p(Y_t ∣ y_t, \phi), \quad t > 0
\end{aligned}
$$
...
<!-- $$
\begin{aligned}
\textcolor{blue}{Y} &= (\textcolor{red}{Y_1}, \dotsc, \textcolor{red}{Y_n}) \\
\textcolor{red}{Y_t} &\sim \textcolor{teal}{\mathcal{D}}(y_t, \phi) \\
\textcolor{teal}{\mathcal{D}} &= \text{NegBin} \\
\textcolor{orange}{\phi} &\sim \mathcal{N}(10, 5) \\
\textcolor{purple}{y_t} &= \textcolor{brown}{\alpha_t} \sum_{s<t}\textcolor{magenta}{i_s} \pi_{t-s} \quad t > 0 \\
\textcolor{brown}{\alpha_t} &= g^{-1}(\eta_t) \\
g^{-1}(x) &= \frac{1}{1 + e^{-x}} \quad g^{-1}: \mathbb{R} \to \mathbb{R}\\
\textcolor{olive}{\eta_t} &= \beta_0 + \sum_{i=1}^9 \beta_i x_i \\
\textcolor{cyan}{\beta_0} &\sim \mathcal{N}(-6.5, 0.5) \\
\textcolor{cyan}{\beta_1}, \dotsc, \textcolor{cyan}{\beta_6} &\sim \mathcal{N}(0, 0.25) \\
\textcolor{cyan}{\beta_7} &\sim \mathcal{N}(-0.2, 0.2) \\
\textcolor{cyan}{\beta_8} &\sim \mathcal{N}(0, 0.2) \\
\textcolor{cyan}{\beta_9} &\sim \mathcal{N}(-50, 0.01) \\
\pi &= \begin{bmatrix} 0.05 & 0.1 &  0.175 & 0.225 &  0.175 &  0.125 & 0.075 &  0.05 & 0.025 \end{bmatrix} \\
\textcolor{magenta}{i_t} &= \textcolor{violet}{\mathscr{R}_t} \sum_{s < t} \textcolor{magenta}{i_s} g_{t-s} \\
\textcolor{magenta}{i_{v:0}}, \textcolor{violet}{\mathscr{R}}, \textcolor{orange}{\phi}, \textcolor{brown}{\alpha} &\sim p(\cdot) \\
\textcolor{violet}{\mathscr{R}} &= (\textcolor{violet}{\mathscr{R}_1}, \dotsc, \textcolor{violet}{\mathscr{R}_n}) \\
\textcolor{brown}{\alpha} &= (\textcolor{brown}{\alpha_1}, \dotsc, \textcolor{brown}{\alpha_n}) \\
p(\textcolor{magenta}{i_{v:0}},\textcolor{violet}{\mathscr{R}},\textcolor{orange}{\phi},\textcolor{brown}{\alpha}∣\textcolor{blue}{Y}) &\propto p(\textcolor{magenta}{i_{v:0}})p(\textcolor{violet}{\mathscr{R}})p(\textcolor{orange}{\phi})p(\textcolor{brown}{\alpha})\prod_t p(\textcolor{red}{Y_t}∣\textcolor{purple}{y_t},\textcolor{orange}{\phi})
\end{aligned}
$$
...
![Example transmission component, i.e. $\mathscr{R}_t$ estimates, using `cfaepim` presets. This is the `R_ts` in the Python code example.](./source/figures/transmission_eg.png){ width=100% }
...
Use parameters and priors
  For transmission to define math of transmission
  For observation to define math of observation
  To then write entire model, w/ presets
Then begin with entire model
  Walk through loading the data
-->
