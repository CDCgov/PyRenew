...
We begin by loading `numpyro` and configuring the device count to 2 to enable running MCMC chains in parallel. By default, XLA (which is used by JAX for compilation) considers all CPU cores as one device. We use numpyro's [set_host_device_count](https://num.pyro.ai/en/stable/utilities.html#set-host-device-count) function to specify the number of device available. 
