...
az.plot_ppc(idata, kind="kde", ax=ax)
ax.legend()
plt.show()
```

```{python}
posterior_predictive_samples = idata.posterior_predictive["negbinom_rv"]
time = idata.posterior_predictive["negbinom_rv_dim_0"].values
hdi_data = az.hdi(posterior_predictive_samples, hdi_prob=0.94)
fig, ax = plt.subplots(figsize=(10, 6))
az.plot_hdi(
    time, posterior_predictive_samples, ax=ax, hdi_prob=0.94, color="lightblue"
)
ax.plot(
    time, daily_hosp_admits, color="black", label="Observed Data", linewidth=2
)
posterior_mean = posterior_predictive_samples.mean(dim=["chain", "draw"])
ax.plot(
    time,
    posterior_mean,
    color="orange",
...
ax.set_xlabel("Time (negbinom_rv_dim_0)")
ax.set_ylabel("Value")
ax.set_xlim(1, daily_hosp_admits.size)
ax.legend()
plt.show()
```
...
```{python}
# other example that works but doesn't use arviz to plot in the way we desire
posterior_predictive_samples = idata.posterior_predictive["negbinom_rv"]
time = idata.posterior_predictive["negbinom_rv_dim_0"].values
fig, ax = plt.subplots(figsize=(10, 6))
num_pp_samples = 100
for i in range(num_pp_samples):
    ax.plot(
        time, posterior_predictive_samples[0, i, :], color="blue", alpha=0.1
    )
ax.plot(
    time, daily_hosp_admits, color="black", label="Observed Data", linewidth=2
)
