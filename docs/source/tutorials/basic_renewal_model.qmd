...
```
By default, XLA (which is used by JAX for compilation) considers all CPU cores as one device. Depending on your system's configuration, we recommend using [numpyro's](https://num.pyro.ai/en/stable/utilities.html#set-host-device-count) `set_host_device_count` to set the number of device available for parallel computing. Here, we set the device count to be 2.
```{python}
