...
```
Below we plot $90 \%$ and $50 \%$ highest density interval for latent hospital admissions using [plot_hdi](https://python.arviz.org/en/stable/api/generated/arviz.plot_hdi.html):
...
mean_latent_hosp_admission = az.summary(idata.posterior['latent_hospital_admissions'])['mean']
axes.plot(x_data,mean_latent_hosp_admission, color='C0', label='Mean')
axes.legend()
axes.set_xlabel('Time', fontsize=10)
axes.set_ylabel('Posterior distribution (hospital admissions)',fontsize=10);
```
...
# | fig-cap: Latent infections posterior distribution
