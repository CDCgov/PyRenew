...
We can visualize the distribution of latent hospital admissions using:
```{python}
...
fig, axes = plt.subplots(figsize=(5,22))
az.plot_forest(
    idata.posterior['latent_hospital_admissions'],
    hdi_prob=0.95,
    ax=axes
);
```
...
fig, axes = plt.subplots(figsize=(5,22))
az.plot_forest(
    idata.posterior['all_latent_infections'],
    hdi_prob=0.95,
    ax=axes
);
```
