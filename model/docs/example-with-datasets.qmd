...
Were $d(\tau)$ is the infection to hospitalization interval, $I(t)$ is the number of latent infections at time $t$, $p_\mathrm{hosp}(t)$ is the infection to hospitalization rate, and $\omega(t)$ is the weekday effect at time $t$; the last section provides an example building a weekly effect `RandomVariable`.
...
inf_hosp_int = deterministic.DeterministicPMF(inf_hosp_int, tol = 1e-2)
...
from pyrenew.latent.infection_seeding_method import SeedInfectionsZeroPad
...
hosp_model.run(
    num_samples=2000,
    num_warmup=2000,
    n_timepoints=dat_w_padding.shape[0] - 16,
