...
The first half of the model is not looking good. The reason is that the infection to hospitalization interval PMF makes it unlikely to observe admissions from the beginning. To solve this, we can use the padding argument to add a few days of missing data at the beginning of the model. The following code will add 21 days of missing data at the beginning of the model:
