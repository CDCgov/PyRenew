...
Where $h(t)$ is the observed number of hospital admissions at time $t$, and $H(t)$ is the number of latent hospital admissions at time $t$. The distribution $\text{HospDist}$ can be any discrete distribution, but we will use a negative binomial distribution.
...
```{python}
#| label: plot-hospital admissions
...
```{python}
#| label: output-hospital admissions
...
```{python}
#| label: output-hospital admissions-weekday
#| fig-cap: Hospital Admissions posterior distribution
#| fig-cap-location: bottom
