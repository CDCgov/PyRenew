...
Below we use `plot_trace` to inspect the trace of the first $20$ $R_t$ estimates.
...
az.plot_trace(
    idata.posterior,
    var_names=['Rt'],
    coords={'Rt_dim_0': np.arange(19)},
    compact=False,
);
```

and now the remaining $R_t$ estimates:
...
az.plot_trace(
    idata.posterior,
    var_names=['Rt'],
    coords={'Rt_dim_0': np.arange(19,34)},
    compact=False,
);
```

We inspect the posterior distribution of $R_t$ by plotting the $90 \%$ and $50 \%$ highest density interval:

```{python}
#| label: fig-hdi-Rt
#| fig-cap: High density interval for Rt estimates
...
mean_Rt = az.summary(idata.posterior['Rt'])['mean']
axes.plot(x_data,mean_Rt, color='C0', label='Mean $R_t$')
axes.legend()
axes.set_xlabel('Time', fontsize=10)
axes.set_ylabel('Posterior distribution ($R_t$)', fontsize=10);
```
...
#Add mean of the posterior to the figure
mean_latent_infection = az.summary(idata.posterior['all_latent_infections'])['mean']
axes.plot(x_data,mean_latent_infection, color='C0', label='mean')
axes.legend()
axes.set_xlabel('Time', fontsize=10)
axes.set_ylabel('Posterior distribution (latent infections)',fontsize=10);
```
