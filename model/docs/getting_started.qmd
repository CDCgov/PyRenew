...
Below we use `plot_trace` to inspect the trace of the $R_t$ estimates.

```{python}
#| label: fig-trace-Rt
#| fig-cap: Trace plot of Rt posterior distribution

az.plot_trace(idata.posterior["Rt"],compact=True);
#set compact=False for separate trace for each Rt
```

We can inspect distribution of the $R_t$ estimates
...
axes.legend()
axes.set_ylabel('$R_t$ estimates',fontsize=10);
```
