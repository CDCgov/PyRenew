...
We can also use ArviZ package to create model diagnostics and visualizations. Below we use `plot_trace` to inspect the trace of daily $R_t$ estimates.
...
We can inspect distribution of daily $R_t$ estimates

```{python}
#| label: viz_forest
#| fig-cap: Daily Rt estimates
az.plot_forest(idata.posterior['Rt'],hdi_prob=0.95);
```
