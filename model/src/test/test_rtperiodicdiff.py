...
def test_rtweeklydiff() -> None:
    ...
    rtwd = RtWeeklyDiffProcess(**params)
    with npro.handlers.seed(rng_seed=np.random.randint(1, 600)):
        rt2 = rtwd.sample(duration=duration).rt.array
...
def test_rtweeklydiff_manual_reconstruction() -> None:
    ...
    _, ans0 = lax.scan(
        ...
        init=np.hstack([params["log_rt_prior"].sample()[0].array, b]),
        xs=noise,
    )

    ans1 = _manual_rt_weekly_diff(
        log_seed=params["log_rt_prior"].sample()[0].array, sd=noise, b=b
    )
...
def test_rtperiodicdiff_smallsample():
    ...
    np.random.seed(223)
    with npro.handlers.seed(rng_seed=np.random.randint(1, 600)):
        rt = rtwd.sample(duration=6).rt.array
